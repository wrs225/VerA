...
def fixed_point_expr(reg,expr):
    ...
    if isinstance(expr, Product):
    ...
    elif isinstance(expr, Quotient): #added by will
        lhse = rec(expr.lhs)
        rhse = rec(expr.rhs)
        lhse, rhse, result_type = expr_type_match(lhse, rhse)
    ...
    elif isinstance(expr, Reciprocal):
        exprr = rec(expr.expr)
        expr_type = reg.get_type(expr.ident)
    ...
    elif isinstance(expr, Sum): #improved by will
        lhse = rec(expr.lhs)
        rhse = rec(expr.rhs)
        lhse, rhse, result_type = expr_type_match(lhse, rhse)
        prod = Sum(lhse, rhse)
    ...
    elif isinstance(expr, Difference): #improved by will
        lhse = rec(expr.lhs)
        rhse = rec(expr.rhs)
        ...
        prod = Difference(lhse_tm, rhse_tm)
        prod.type = targ_type
        return prod
    ...
    elif isinstance(expr, VarAssign):
        lhse =  rec(expr.lhs)
        rhse = rec(expr.rhs)
        ...
        return VarAssign(lhse, rhse_tm)

    elif isinstance(expr, Integrate):
        lhse =  rec(expr.lhs)
        rhse = rec(expr.rhs)
        rhse_tm = type_match(rhse,lhse.type)
        return Accumulate(lhse, rhse_tm)
...
def fixed_point_var(reg,var):
    if var.type.isType(RealType):
        typ = reg.get_type(var.name)
        return typ
    elif var.type.isType(DigitalType):
        typ = reg.get_type(var.name)
        return typ
    else:
        return var
...
def fixed_point_block(reg,block):
    fp_block = AMSBlock(block.name + "_fp")
    for v in block.vars():
